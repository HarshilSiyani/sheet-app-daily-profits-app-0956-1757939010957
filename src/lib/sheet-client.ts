...
// Auto-generated for: Daily Profits App
...
export class SheetClient {
  ...
  constructor() {
    ...
    this.apiKey = process.env.NEXT_PUBLIC_SHEETAPPS_API_KEY || 'sk_47af928b96434473a8e159811a95ab9c';
    this.sheetId = process.env.NEXT_PUBLIC_SHEET_ID || '1lq5RkslA1IkPRqgGHQDnhFJ3z1JOfXbHwRGs4VguBB0';
  ...
  async addRow(data: Record<string, any>): Promise<ApiResponse> {
    try {
      const headers = ['StoreName', 'Date', 'Profit'];
  ...
  async updateField(params: UpdateOperation): Promise<ApiResponse> {
    try {
      ...
      const columnIndex = ['StoreName', 'Date', 'Profit'].indexOf(params.field);
